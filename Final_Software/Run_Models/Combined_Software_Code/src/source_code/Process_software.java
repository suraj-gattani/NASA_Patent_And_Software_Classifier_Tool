...
public class Process_software {
	...
	public static String smo(String filename) throws Exception {
		...
		try {
			...
			InputStream input = Process_software.class.getResourceAsStream("/resources/" + filename);
			...
			InputStream input1=	Process_software.class.getResourceAsStream("/resources/"+"stop_words.txt");
			...
			File fc1_model=new File(absPathFile.getAbsolutePath()+prop.getProperty("NasaWorkingDirectory")+"fc_software.model");
			InputStream input2=	Process_software.class.getResourceAsStream("/resources/"+"fc_software.model");
			...
			File ac_model=new File(absPathFile.getAbsolutePath()+prop.getProperty("NasaWorkingDirectory")+"ac_software.model");
			InputStream input3=	Process_software.class.getResourceAsStream("/resources/"+"ac_software.model");
			...
			File SMO_model=new File(absPathFile.getAbsolutePath()+prop.getProperty("NasaWorkingDirectory")+"SMO_software.model");
			InputStream input4=	Process_software.class.getResourceAsStream("/resources/"+"SMO_software.model");
			...
			File trainFinal=new File(absPathFile.getAbsolutePath()+prop.getProperty("NasaWorkingDirectory")+"train_software.arff");
			InputStream input5=	Process_software.class.getResourceAsStream("/resources/"+"train_software.arff");
		...
		ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream(absPathFile.getAbsolutePath()+prop.getProperty("NasaWorkingDirectory")+"fc_software.model"));
		...
		Filter ac= (Filter) weka.core.SerializationHelper.read(absPathFile.getAbsolutePath()+prop.getProperty("NasaWorkingDirectory")+"ac_software.model");
		...
		BufferedReader readerTrain = new BufferedReader(new FileReader(absPathFile.getAbsolutePath()+prop.getProperty("NasaWorkingDirectory")+"train_software.arff"));
		...
		cls.setModelFile(new File(absPathFile.getAbsolutePath()+prop.getProperty("NasaWorkingDirectory")+"SMO_software.model"));
		...
		for (int i = 0; i < fcdlist.length; i++) {
			Probability_TopClass_Software.setclasslist(classlist);
			Probability_TopClass_Software.setfcdist(fcdlist);
		}
		Probability_TopClass_Software.probabilityCL();
		
		for (int i = 0; i < testFinal.numInstances(); i++) {
			double pred = cls.classifyInstance(testFinal.instance(i));
			Nasa_Main.setclassname_software(testFinal.classAttribute().value((int) pred));
